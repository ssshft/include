...
int main(int argc, const char** argv) {
    ...
    while(true) {
        for(int i = 1; i < argc; i++) {
            ...
            if(res == MsgQ::ReadOK) {
                uint64_t now1 = now();//rdtsc();
                MsgQ::MsgHeader* header = (MsgQ::MsgHeader*)buf;
                auto msg_type = header->userdata;
                switch(msg_type) {
                    case 1: handleMsg<Msg1>(header, argv[i], now1); break;
                    case 2: handleMsg<Msg2>(header, argv[i], now1); break;
                    case 3: handleMsg<Msg3>(header, argv[i], now1); break;
                    case 4: handleMsg<Msg4>(header, argv[i], now1); break;
